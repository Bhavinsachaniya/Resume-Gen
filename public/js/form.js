...
finish.addEventListener('click', async () => {
    ...
    let contactfirstname = document.querySelector(
        'input[name="contactfirstname"]'
    ).value;
    let contactlastname = document.querySelector(
        'input[name="contactlastname"]'
    ).value;
    let contactcity = document.querySelector('input[name="contactcity"]').value;
    let contactpostalcode = document.querySelector(
        'input[name="contactpostalcode"]'
    ).value;
    let contactphone = document.querySelector('input[name="contactphone"]').value;
    let contactemail = document.querySelector('input[name="contactemail"]').value;

    //*experience

    let jobTitle = Array.from(
        document.querySelectorAll('input[name="jobTitle"]')
    ).map((input) => input.value);
    let companyName = Array.from(
        document.querySelectorAll('input[name="companyName"]')
    ).map((input) => input.value);
    let expCity = Array.from(
        document.querySelectorAll('input[name="expCity"]')
    ).map((input) => input.value);
    let country = Array.from(
        document.querySelectorAll('input[name="country"]')
    ).map((input) => input.value);
    let startDate = Array.from(
        document.querySelectorAll('input[name="startDate"]')
    ).map((input) => input.value);
    let endDate = Array.from(
        document.querySelectorAll('input[name="endDate"]')
    ).map((input) => input.value);
    ...
    let eduschool = Array.from(
        document.querySelectorAll('input[name="eduschool"]')
    ).map((input) => input.value);
    let edudegree = Array.from(
        document.querySelectorAll('input[name="edudegree"]')
    ).map((input) => input.value);
    let educity = Array.from(
        document.querySelectorAll('input[name="educity"]')
    ).map((input) => input.value);
    let educountry = Array.from(
        document.querySelectorAll('input[name="educountry"]')
    ).map((input) => input.value);
    let edustartdate = Array.from(
        document.querySelectorAll('input[name="edustartdate"]')
    ).map((input) => input.value);
    let eduenddate = Array.from(
        document.querySelectorAll('input[name="eduenddate"]')
    ).map((input) => input.value);
    let edugpa = Array.from(document.querySelectorAll('input[name="eduGPA"]')).map((input) => input.value);

    //*Project data

    let projecttitle = Array.from(document.querySelectorAll('input[name="ProjectTitle"]')).map((input) => input.value) ;
    let projectStartDate = Array.from(document.querySelectorAll('input[name="ProjectStartDate"]')).map((input) => input.value);
    let projectEndDate = Array.from(document.querySelectorAll('input[name="ProjectEndDate"]')).map((input) => input.value);
    let projectoverview = Array.from(document.querySelectorAll('textarea[name="Project-overview"]')).map((textarea) => textarea.value);

    //* achievements data

    let Achievementstitle  = Array.from(document.querySelectorAll('input[name="Achievements"]')).map((input) => input.value);
    // let Achievementslink = Array.from(document.querySelectorAll('input[name="Achievements-link"]')).map((input) => input.value);

    //* hobbies data

    let hobbies =  Array.from(document.querySelectorAll('input[name="hobbies"]')).map((input) => input.value);
    
    //* skills data

    let skill = Array.from(
        document.querySelectorAll('input[name="skill"]')
    ).map((input) => input.value);

    //* about data

    let abouttext = document.querySelector('textarea[name="abouttext"]').value;

    //* for the access the templatenumber
    ...
    const templateNumber = parts[parts.length - 1];

    console.log('Template number is:', templateNumber);

    

    try {
        const response = await fetch('http://localhost:3000/resumeform/data', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            // Now we handle it before throwing
            let alert = 'Failed to submit form';
            showAlert(alert, 'error');
            throw new Error(alert); // You can throw after showing the alert
        }
        ...
        try {
            ...
            a.href = url;
            a.download = `generated_resume_${contactfirstname}.docx`;
            ...
            showAlert('An error occurred while downloading the resume.', 'error');
        }
...
/* 
        ...
        try {
            const response = await fetch('http://localhost:3000/resumeform/datasave', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            resumeId = result.resumeId; // You need backend to return this
            showAlert('Data saved successfully!', 'success');
        } catch (error) {
            console.error('Submission error:', error.message);
            return showAlert('An error occurred while saving data.', 'error');
        }
        


        try {
            const response = await fetch(`http://localhost:3000/resumeform/generate/${resumeId}`);
            if (!response.ok) {
                showAlert('Failed to generate resume', 'error');
...
finish.addEventListener('click', async () => {
    ...
    // Contact details
    let contactfirstname = document.querySelector('input[name="firstName"]').value;
    let contactlastname = document.querySelector('input[name="lastName"]').value;
    let contactcity = document.querySelector('input[name="city"]').value;
    let contactpostalcode = document.querySelector('input[name="postal"]').value;
    let contactphone = document.querySelector('input[name="phone"]').value;
    let contactemail = document.querySelector('input[name="email"]').value;

    // Experience data
    let jobTitle = Array.from(document.querySelectorAll('input[name="jobTitle"]')).map(input => input.value);
    let companyName = Array.from(document.querySelectorAll('input[name="company"]')).map(input => input.value);
    let expCity = Array.from(document.querySelectorAll('input[name="jobCity"]')).map(input => input.value);
    let country = Array.from(document.querySelectorAll('input[name="jobCountry"]')).map(input => input.value);
    let startDate = Array.from(document.querySelectorAll('input[name="jobStart"]')).map(input => input.value);
    let endDate = Array.from(document.querySelectorAll('input[name="jobEnd"]')).map(input => input.value);
    let descriptions = Array.from(document.querySelectorAll('textarea[name="jobDesc"]')).map(textarea => textarea.value);

    // Education data
    let eduschool = Array.from(document.querySelectorAll('input[name="eduschool"]')).map(input => input.value);
    let edudegree = Array.from(document.querySelectorAll('input[name="edudegree"]')).map(input => input.value);
    let educity = Array.from(document.querySelectorAll('input[name="educity"]')).map(input => input.value);
    let educountry = Array.from(document.querySelectorAll('input[name="educountry"]')).map(input => input.value);
    let edustartdate = Array.from(document.querySelectorAll('input[name="edustartdate"]')).map(input => input.value);
    let eduenddate = Array.from(document.querySelectorAll('input[name="eduenddate"]')).map(input => input.value);
    let edugpa = Array.from(document.querySelectorAll('input[name="eduGPA"]')).map(input => input.value);
    console.log("hereherhehreher"+edustartdate);
    // Project data
    let projecttitle = Array.from(document.querySelectorAll('input[name="ProjectTitle"]')).map(input => input.value);
    let projectStartDate = Array.from(document.querySelectorAll('input[name="ProjectStartDate"]')).map(input => input.value);
    let projectEndDate = Array.from(document.querySelectorAll('input[name="ProjectEndDate"]')).map(input => input.value);
    ...
    // Skills data
    let skill = Array.from(document.querySelectorAll('input[name="skills[]"]')).map(input => input.value);

    // About data
    let abouttext = document.querySelector('textarea[name="summary"]').value;

    // Template number (from URL or some other source)
    const currentUrl = window.location.pathname;
    const parts = currentUrl.split('/');
    const templateNumber = parts[parts.length - 1]; // Assume template number is the last part of the URL

    try {
        const response = await fetch('http://localhost:3000/api/resumeform/genrate', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        let alert= 'Resume generated and downloaded successfully!';
        showAlert(alert, 'success');
    } catch (error) {
        console.error('Error:', error.message);
        let alert = 'An error occurred. Please try again.';
        showAlert(alert, 'error');
    }
});
